...
async def language_check(bot, query):
    ...
    if files:
        ...
        # Generate buttons without shortlinks
        if settings['button']:
            btn = [
            ...
            try:
                if settings['max_btn']:
                    btn.append([
                        ...
                        InlineKeyboardButton(text=f"{math.ceil(int(total_results) / 10)}", callback_data="pages"),
                        InlineKeyboardButton(text="𝐍𝐄𝐗𝐓 ➪", callback_data=f"next_{req}_{key}_{offset}")
                    ])
                else:
                    btn.append([
                        InlineKeyboardButton("𝐏𝐀𝐆𝐄", callback_data="pages"),
                        InlineKeyboardButton(text=f"{math.ceil(int(total_results) / int(MAX_B_TN))}", callback_data="pages"),
                        InlineKeyboardButton(text="𝐍𝐄𝐗𝐓 ➪", callback_data=f"next_{req}_{key}_{offset}")
                    ])
            except KeyError:
...
async def pm_text(bot, message):
    ...
    await message.reply_text(
         text="<b>ʜᴇʏ ᴅᴜᴅᴇ 😍 ,\n\nʏᴏᴜ ᴄᴀɴ'ᴛ ɢᴇᴛ ᴍᴏᴠɪᴇs ꜰʀᴏᴍ ʜᴇʀᴇ. ʀᴇǫᴜᴇsᴛ ᴏɴ ᴏᴜʀ <a href=M_GRP_LINK>ᴍᴏᴠɪᴇ ɢʀᴏᴜᴘ</a> ᴏʀ ᴄʟɪᴄᴋ ʀᴇǫᴜᴇsᴛ ʜᴇʀᴇ ʙᴜᴛᴛᴏɴ ʙᴇʟᴏᴡ​👇</b>",   
         reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("📝 ʀᴇǫᴜᴇsᴛ ʜᴇʀᴇ​ ", url=M_GRP_LINK)]]))
...
#ALERT FN IN SPELL CHECK FOR LANGAUGES TO KNOW HOW TO TYPE MOVIES esp english spell check goto adv spell check to check donot change the codes      
        ...
        free = get_size(free)
        await query.message.edit_text(
            text=script.STATUS_TXT.format(uptime,total, users, chats, monsize, free),
            reply_markup=reply_markup,
            parse_mode=enums.ParseMode.HTML
        )
